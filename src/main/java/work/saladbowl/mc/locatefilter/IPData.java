...
public class IPData {
    ...
    public static Boolean getCountry(String IPAddr) throws UnknownHostException {
        ...
        if(config.getStringList("blockIPArr").contains(IPAddr)) {
            ...
            String[] locales = config.getStringList("allowCountry").toArray(new String[config.getStringList("allowCountry").size()]);
            for (String locale: locales) {
                ...
                try {
                    ...
                    Bukkit.getLogger().info("[Locate Filter ERROR]" + e.toString());
                }
            }
            return false;
        }
